...
import gtintel.dao.DBUtil;
import gtintel.dao.DataDicDaoImpl;
...
public class DataDictionaryService {
	...
	public void getlist(@Context HttpServletRequest  req) throws InstantiationException, IllegalAccessException, ClassNotFoundException, IllegalArgumentException, InvocationTargetException{
		
		Map<String,String[]> map = req.getParameterMap();
		Map<String,Object> _map = new HashMap<>();
		for (Map.Entry<String,String[]> entry : map.entrySet()) {
			//if (!entry.getKey().equals("itfName")) {
				String key = entry.getKey();
				String value = entry.getValue()[0];
				_map.put(key, value);
				System.out.println("Key = " + key + ", Value = " + value);
			//}else{
			//itfName = entry.getValue()[0];
			//}
		}
		 DataDicDaoImpl dimp = new DataDicDaoImpl();
		 
		 String itfName =  (String) _map.get("itfName");
		//反射
		//Class<?> clazz = Class.forName("gtintel.dictionary.entity."+itfName);
		Connection conn = DBUtil.getConn();
	    String sql = "insert into t_"+itfName+" (";
